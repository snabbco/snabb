...
function ConnectX:new (conf)
   ...
   if macvlan then
      ...
      local flow_group_ip =
         hca:create_flow_group_ip(rxtable, NIC_RX, index,
                                  index + #l3_protos * #l4_protos - 1)
...
function HCA:set_flow_table_entry_ip (table_id, table_type, group_id,
   ...
   if l4_proto ~= nil then
      proto = assert(l4_protos[l4_proto], "invalid l4 proto")
      self:input("match_proto",  0x40 + 0x40 + 0x10, 31, 24, proto)
   end
   self:input("dest_type",    0x40 + 0x300, 31, 24, 2) -- TIR
      :input("dest_id",      0x40 + 0x300, 23,  0, tir)
      :execute()
end
