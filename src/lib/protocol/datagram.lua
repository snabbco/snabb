...
-- packet's payload.  The basic method parse() takes two arguments,
-- which can both be nil.
--
...
-- part of the packet.  If the class is nil, the current ULP of the
-- packet is used.
--
-- If the current ULP is not set (nil) or the constructor of the
...
function datagram:parse (class, check)
...
-- This method is a wrapper for parse() that allows parsing of a
...
-- calls to parse() return nil.
function datagram:parse_seq (seq)
   local proto = nil
   local i = 1
   while seq[i] do
      proto = self:parse(seq[i][1], seq[i][2])
...
-- This method is a wrapper for parse() that parses the next n
