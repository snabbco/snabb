...
timeout 5 ./snabb packetblaster replay --duration 1 replay program/snabbnfv/test_fixtures/pcap/64.pcap ${SNABB_PCI0}
status=$?
if [ $status != 124 ]; then
...
timeout 5 ./snabb packetblaster synth --src 11:11:11:11:11:11 --dst 22:22:22:22:22:22 --sizes 64,128,256 --duration 1 ${SNABB_PCI0}
status=$?
if [ $status != 124 ]; then
    echo "Error: expected timeout (124) but got ${status}"
    exit 1
