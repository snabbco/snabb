...
static CTypeID register_intrinsic(lua_State *L, AsmIntrins* src, CType *func)
{
  ...
  if ((cts->intr.top+1) > cts->intr.sizetab) {
    lj_mem_growvec(cts->L, cts->intr.tab, cts->intr.sizetab, LJ_INTRINS_MAXID, AsmIntrins);
  }
...
int lj_intrinsic_create(lua_State *L)
{
  ...
  void *intrinsmc;
  MSize asmsz;
  AsmIntrins* intrins = lj_intrinsic_get(cts, ctype_get(cts, id)->size);
...
{
  GCcdata *cd;
  AsmIntrins *intrins = lj_intrinsic_get(cts, func->size);
...
{
  lua_State *L = cts->L;
  AsmIntrins *intrins = lj_intrinsic_get(cts, ct->size);
