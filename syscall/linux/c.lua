...
if abi.abi32 then
  if zeropad then
    ...
    function C.pread(fd, buf, size, offset)
      ...
      return syscall(c.SYS.pread64, t.int(fd), pt.void(buf), t.size(size), t.int(0), t.long(off1), t.long(off2))
    end
    function C.pwrite(fd, buf, size, offset)
      local off1, off2 = arg64(offset)
      return syscall(c.SYS.pwrite64, t.int(fd), pt.void(buf), t.size(size), t.int(0), t.long(off1), t.long(off2))
    end
  else
    function C.truncate(path, length)
      local len1, len2 = arg64u(length)
    ...
    function C.pread(fd, buf, size, offset)
      local off1, off2 = arg64(offset)
      return syscall(c.SYS.pread64, t.int(fd), pt.void(buf), t.size(size), t.long(off1), t.long(off2))
    end
    function C.pwrite(fd, buf, size, offset)
      local off1, off2 = arg64(offset)
  ...
  function C.preadv(fd, iov, iovcnt, offset)
    ...
    return syscall(c.SYS.preadv, t.int(fd), pt.void(iov), t.int(iovcnt), t.long(off2), t.long(off1))
  end
  function C.pwritev(fd, iov, iovcnt, offset)
    local off1, off2 = llarg64(offset)
    return syscall(c.SYS.pwritev, t.int(fd), pt.void(iov), t.int(iovcnt), t.long(off2), t.long(off1))
  end
  ...
  function C.sendfile(outfd, infd, offset, count)
    return syscall(c.SYS.sendfile64, t.int(outfd), t.int(infd), pt.void(offset), t.size(count))
  end
...
function C.clone(flags, signal, stack, ptid, tls, ctid)
  return syscall(c.SYS.clone, t.int(flags), pt.void(stack), pt.void(ptid), pt.void(tls), pt.void(ctid))
end
...
function C.ioctl(fd, request, arg)
  return syscall(c.SYS.ioctl, t.int(fd), t.uint(request), pt.void(arg))
end
...
function C.getcpu(cpu, node, tcache)
  return syscall(c.SYS.getcpu, pt.uint(node), pt.uint(node), pt.void(tcache))
end
...
if abi.abi64 then
  function C.stat(path, buf)
    return syscall(c.SYS.stat, path, pt.void(buf))
  end
  function C.lstat(path, buf)
    return syscall(c.SYS.lstat, path, pt.void(buf))
  end
  function C.fstat(fd, buf)
    return syscall(c.SYS.fstat, t.int(fd), pt.void(buf))
  end
  function C.fstatat(fd, path, buf, flags)
    return syscall(c.SYS.fstatat, t.int(fd), path, pt.void(buf), t.int(flags))
  end
  ...
  function C.stat(path, buf)
    return syscall(c.SYS.stat64, path, pt.void(buf))
  end
  function C.lstat(path, buf)
    return syscall(c.SYS.lstat64, path, pt.void(buf))
  end
  function C.fstat(fd, buf)
    return syscall(c.SYS.fstat64, t.int(fd), pt.void(buf))
  end
  function C.fstatat(fd, path, buf, flags)
    return syscall(c.SYS.fstatat64, t.int(fd), path, pt.void(buf), t.int(flags))
  end
  if zeropad then
    function C.fadvise64(fd, offset, len, advise)
...
function C.io_setup(nr_events, ctx)
  return syscall(c.SYS.io_setup, t.uint(nr_events), pt.void(ctx))
end
function C.io_destroy(ctx)
  return syscall(c.SYS.io_destroy, t.aio_context(ctx))
end
function C.io_cancel(ctx, iocb, result)
  return syscall(c.SYS.io_cancel, t.aio_context(ctx), pt.void(iocb), pt.void(result))
end
function C.io_getevents(ctx, min, nr, events, timeout)
  return syscall(c.SYS.io_getevents, t.aio_context(ctx), t.long(min), t.long(nr), pt.void(events), pt.void(timeout))
end
function C.io_submit(ctx, iocb, nr)
  return syscall(c.SYS.io_submit, t.aio_context(ctx), t.long(nr), pt.void(iocb))
end
...
function C.prlimit64(pid, resource, new_limit, old_limit)
  return syscall(c.SYS.prlimit64, t.pid(pid), t.int(resource), pt.void(new_limit), pt.void(old_limit))
end
...
function C.sched_getaffinity(pid, len, mask)
  return syscall(c.SYS.sched_getaffinity, t.pid(pid), t.uint(len), pt.void(mask))
end
function C.sched_setaffinity(pid, len, mask)
  return syscall(c.SYS.sched_setaffinity, t.pid(pid), t.uint(len), pt.void(mask))
end
...
function C.sched_getparam(pid, param)
  return syscall(c.SYS.sched_getparam, t.pid(pid), pt.void(param))
end
function C.sched_setparam(pid, param)
  return syscall(c.SYS.sched_setparam, t.pid(pid), pt.void(param))
end

-- in librt for glibc but use syscalls instead of loading another library
function C.clock_nanosleep(clk_id, flags, req, rem)
  return syscall(c.SYS.clock_nanosleep, t.clockid(clk_id), t.int(flags), pt.void(req), pt.void(rem))
end
function C.clock_getres(clk_id, ts)
  return syscall(c.SYS.clock_getres, t.clockid(clk_id), pt.void(ts))
end
function C.clock_gettime(clk_id, ts)
  return syscall(c.SYS.clock_gettime, t.clockid(clk_id), pt.void(ts))
end
function C.clock_settime(clk_id, ts)
  return syscall(c.SYS.clock_settime, t.clockid(clk_id), pt.void(ts))
end
...
function C.utimensat(fd, path, times, flags)
  return syscall(c.SYS.utimensat, t.int(fd), pt.void(path), pt.void(times), t.int(flags))
end

-- not in Android Bionic
function C.linkat(olddirfd, oldpath, newdirfd, newpath, flags)
  return syscall(c.SYS.linkat, t.int(olddirfd), pt.void(oldpath), t.int(newdirfd), pt.void(newpath), t.int(flags))
end
function C.symlinkat(oldpath, newdirfd, newpath)
  return syscall(c.SYS.symlinkat, pt.void(oldpath), t.int(newdirfd), pt.void(newpath))
end
function C.readlinkat(dirfd, pathname, buf, bufsiz)
  return syscall(c.SYS.readlinkat, t.int(dirfd), pt.void(pathname), pt.void(buf), t.size(bufsiz))
end
function C.inotify_init1(flags)
  return syscall(c.SYS.inotify_init1, t.int(flags))
end
function C.adjtimex(buf)
  return syscall(c.SYS.adjtimex, pt.void(buf))
end
...
function C.epoll_wait(epfd, events, maxevents, timeout)
  return syscall(c.SYS.epoll_wait, t.int(epfd), pt.void(events), t.int(maxevents), t.int(timeout))
end
function C.swapon(path, swapflags)
  return syscall(c.SYS.swapon, pt.void(path), t.int(swapflags))
end
function C.swapoff(path)
  return syscall(c.SYS.swapoff, pt.void(path))
end
...
function C.timerfd_settime(fd, flags, new_value, old_value)
  return syscall(c.SYS.timerfd_settime, t.int(fd), t.int(flags), pt.void(new_value), pt.void(old_value))
end
